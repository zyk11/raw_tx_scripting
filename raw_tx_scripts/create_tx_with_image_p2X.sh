...
# Automating the creation of a P2PKH / P2SH transaction with arbitrary image
# Transaction output type is determined by Bitcoin address prefix from the hex byte input file
...
main(){
	if [ "$1" ]
	then
		echo "`date '+%Y-%m-%d %H:%M:%S'` [*] Generating Bitcoin addresses representing the image"
		generate_addresses $1
		echo "`date '+%Y-%m-%d %H:%M:%S'` [*] Creating raw transaction"
		create_raw_tx
	else
		echo "`date '+%Y-%m-%d %H:%M:%S'` [-] Image hex input expected"
	fi
}
